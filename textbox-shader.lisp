...
(defun text-box:create (text-box (str (ptr char)) (max-width i32) 
			(font-size i32) (font (ptr tt:fontinfo)))
  (let ((s :type size)
	(box :type text-box))
    (zeroize box)
    (zeroize s)
    ...
    (tt:iterate str font-size max-width font rect-calc-callback (cast (addrof s) (ptr void)))
    (let ((item :type rect-draw-item))
      ...
      (tt:iterate str font-size max-width font rect-draw-callback (cast (addrof item) (ptr void)))
      (gl:tex-parameter gl:texture-2d gl:texture-min-filter gl:linear)
