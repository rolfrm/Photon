...
(let ((buf2 (cast (alloc (* 2 (size-of (type f32)) (cast circ-pts u64))) (ptr f32))))
  (range it 0 (cast circ-pts i64)
	 (setf (deref (ptr+ buf2 (+ (* it 2) 0))) 
	   (sin32 (* 0.1 (cast it f32))))
     (setf (deref (ptr+ buf2 (+ (* it 2) 1))) 
  ...
  (let ((s (* (size-of (type f32)) (cast circ-pts u64) (cast 2 u64))))
    ...
    (range it 0 (cast circ-pts i64)
	   (print (deref (ptr+ buf2 it)) newline))
...
(add-to-list+ circles circle-cnt (vec 10 40 0.5))
(add-to-list+ circles circle-cnt (vec 10 40 1))
(add-to-list+ circles circle-cnt (vec 10 40 2))
(add-to-list+ circles circle-cnt (vec 10 40 3))
(add-to-list+ circles circle-cnt (vec 10 40 4))
(add-to-list+ circles circle-cnt (vec 10 40 5))
(add-to-list+ circles circle-cnt (vec 10 40 6))
(add-to-list+ circles circle-cnt (vec 10 40 7))
(add-to-list+ circles circle-cnt (vec 10 40 8))
(add-to-list+ circles circle-cnt (vec 10 40 9))
(add-to-list+ circles circle-cnt (vec 10 40 10))
(add-to-list+ circles circle-cnt (vec 10 50 20))
(add-to-list+ circles circle-cnt (vec 10 60 30))
(add-to-list+ circles circle-cnt (vec 10 70 40))
(add-to-list+ circles circle-cnt (vec 10 80 50))
  ...
  (while (< iteration 40000)
	...
	(load-points))
      ...
      (range it 0 (cast circle-cnt i64)
	     (let ((circle (deref (ptr+ circles it))))
